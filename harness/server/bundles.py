...
def runtime_bundle(pairing, origin, image='', source_root=None):
    ...
    with zipfile.ZipFile(buffer, 'w', zipfile.ZIP_DEFLATED) as archive:
        ...
        if not image:
            patterns = ['src/cad1000/*.py', 'harness/server/*.py', 'harness/server/*.scad', 'harness/server/*.FCMacro', 'harness/server/*.cfg', 'harness/server/guides/*.md', 'harness/server/guides/skills/*.md',
